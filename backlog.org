...
** Extract arguments
*** ExtractFirstParameterOfType<T>(x => _member = x);
*** ExtractParameterByIndex(2, x => _member = x);
*** ExtractParameterByName("name", x => _member = x);


** Signal that takes a flag object representing a single flag, to avoid having to specify a magic string
